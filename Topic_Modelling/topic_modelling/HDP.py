    ...
    for type_of_review in filtered_dict:
        ...
        for quarter in filtered_dict[type_of_review]:
            ...
            for brand in filtered_dict[type_of_review][quarter]:
                ...
                topic_list = hdpmodel.show_topics(num_words = 10, formatted= False)
...
def return_coherence(hdp_model, number_of_topics_limit):
    list_of_coherence = hdp_model.get_coherence_per_topic()
    return list_of_coherence[:number_of_topics_limit]
    ...
    for type_of_review in processed_model_by_brand:
        for quarter in processed_model_by_brand[type_of_review]:
            for brand in processed_model_by_brand[type_of_review][quarter]:
                ...
                # Figure out which model to accept or reject, based on the weight alpha which has to be greater than 0.15
                weight_of_topics = [weight for weight in hdp_model.hdp_to_lda()[0] if weight >= 0.15]
                number_of_topics = len(weight_of_topics)

                # Get list of coherence if they are in the number of topics
                list_of_coherence = return_coherence(hdp_coherence, number_of_topics)
                count += 1
                ...
                if number_of_topics == 0:
                    ...
                    for i in range(number_of_topics):
                        topic = "Topic " + str(i+1)
                        coherence = list_of_coherence[i]
                        topic_list_w_keywords = topic_list[i][1::2]
