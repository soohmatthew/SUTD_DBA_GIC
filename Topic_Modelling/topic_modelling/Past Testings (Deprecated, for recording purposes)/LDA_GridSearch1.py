...
def Preprocessing_deprecated(df, LIST_OF_ADDITIONAL_STOP_WORDS, LIST_OF_COMMON_WORDS):
    ...
    for brand in list_of_brands:
        ...
        doc_of_brand = df[(df['Brand'] == brand)]["User Comment"].tolist()

        #Tokenise words
        doc_of_brand_token = list(sent_to_words(doc_of_brand))
        
        #Lemmatize words
        doc_of_brand_lemma = lemmatization(doc_of_brand_token, allowed_postags=['NOUN', 'ADJ', 'VERB', 'ADV'])

        #Remove stopwords, digits.
        doc_of_brand_stop = []
        for sentence in doc_of_brand_lemma:
            sentence = ' '.join(word for word in sentence.split() if word not in stop_words and not word.isdigit())
            doc_of_brand_stop.append(sentence)

        print(doc_of_brand_stop)
        processed_data[brand] = doc_of_brand_stop
...
def Preprocessing(df, LIST_OF_ADDITIONAL_STOP_WORDS, LIST_OF_COMMON_WORDS):
    ...
    for quarter in list_of_quarters:
        ...
        processed_data_by_quarter[str(quarter)] = doc_of_quarter_stop
        processed_data_by_quarter_by_brand[str(quarter)] = OrderedDict()
        ...
        for brand in list_of_brands:
            ...
            if doc_of_quarter_by_brand == []:
                ...
                processed_data_by_quarter_by_brand[str(quarter)][brand] = doc_of_quarter_by_brand_stop
...
def build_single_LDA_model(dict_of_clean_doc, quarter, brand, number_of_topics_range):
    try:
        ...
        if brand is None:
            ...
            doc_clean = dict_of_clean_doc[str(quarter)]
        else:
            print("Building LDA model for ... {}, {} ".format(str(quarter), brand))
            doc_clean = dict_of_clean_doc[str(quarter)][brand]
        ...
        for topic_number in set(dominant_topic):
...
# def LDA_topic_modeller_by_quarter(DF, LIST_OF_ADDITIONAL_STOP_WORDS, LIST_OF_COMMON_WORDS, number_of_topics_range):
...
#     topic_model_by_quarter = pd.DataFrame()

#     for quarter in list_of_quarters:
        
#         topic_keywords_dict = build_single_LDA_model(dict_of_clean_doc_by_quarter, str(quarter), None, number_of_topics_range)
#         for topic in topic_keywords_dict:
#             for keyword in topic_keywords_dict[topic]['keywords']:
#                 keyword_dict = OrderedDict()
#                 keyword_dict['Quarter'] = str(quarter)
...
#                 keyword_dict['Keyword'] = keyword[0]
#                 keyword_dict['Keyword Weight'] = keyword[1]
#                 topic_model_by_quarter = topic_model_by_quarter.append(keyword_dict, ignore_index=True)
    
#     topic_model_by_quarter.to_excel(writer,'Topic Model by Quarter')
#     writer.save()
#     writer.close()
#     return

# def LDA_topic_modeller_by_brand(DF, LIST_OF_ADDITIONAL_STOP_WORDS, LIST_OF_COMMON_WORDS, number_of_topics_range):
#     if os.path.isfile('pickle_files/{}.pickle'.format('processed_data_by_quarter')) and os.path.isfile('pickle_files/{}.pickle'.format('processed_data_by_quarter_by_brand')): 
...
#     topic_model_by_quarter_by_brand = pd.DataFrame()

#     for quarter in list_of_quarters:
#         for brand in list_of_brands:
#             topic_keywords_brand_dict = build_single_LDA_model(dict_of_clean_doc_by_quarter_by_brand, str(quarter), brand, number_of_topics_range)
    
#     topic_model_by_quarter_by_brand.to_excel(writer,'Topic Model by Quarter by Brand')
#     writer.save()
#     writer.close()
#     return
...
def LDA_topic_modeller_by_quarter_by_brand_multiprocessing(DF, LIST_OF_ADDITIONAL_STOP_WORDS, LIST_OF_COMMON_WORDS, number_of_topics_range):
    ...
    for quarter in list_of_quarters:
        combination_of_brands += list(itertools.product([str(quarter)], dict_of_clean_doc_by_quarter_by_brand[str(quarter)].keys()))

    from multiprocessing import Pool, cpu_count, Manager
    print("{} products found... ".format(str(len(combination_of_brands))))
    list_of_arguments = [(dict_of_clean_doc_by_quarter_by_brand, str(quarter_brand[0]), quarter_brand[1], number_of_topics_range) for quarter_brand in combination_of_brands]
...
if __name__ == "__main__":
    ...
    number_of_topics_range = [1,2,3,4,5]
...
#             dict_of_clean_doc_by_quarter_by_brand = pickle.load(handle_2)
#     else:
#         dict_of_clean_doc_by_quarter, dict_of_clean_doc_by_quarter_by_brand = Preprocessing(DF, LIST_OF_ADDITIONAL_STOP_WORDS, LIST_OF_COMMON_WORDS)

#     combination_of_brands = []
#     for i in dict_of_clean_doc_by_quarter_by_brand:
#         if '2016' in i or '2017' in i or '2018' in i:
#             combination_of_brands += list(itertools.product([i], dict_of_clean_doc_by_quarter_by_brand[i].keys()))

    
#     doc_clean = dict_of_clean_doc_by_quarter_by_brand['2018Q4']['DeLonghi']

#     #Build model
#     #lda_model = build_single_LDA_model(doc_clean, number_of_topics_range)
...
#     with open('lda_model_test.pickle', 'rb') as handle_3:
#         grid_search_model = pickle.load(handle_3)
...
#     topic_keywords_dict = show_topics(vectorizer=vectorizer, lda_model=best_model, n_words=15)
#     lda_output = best_model.transform(data_vectorized)
#     # column names
...
#     #Count of dominant topic of each document
#     for topic_number in set(dominant_topic):
#         topic_keywords_dict['topic ' + str(topic_number + 1)]['frequency'] = dominant_topic.count(topic_number)
...
#         for keyword in topic_keywords_dict[topic]['keywords']:
#             keyword_dict = OrderedDict()
#             keyword_dict['Quarter'] = '2018Q4'
#             keyword_dict['Brand'] = 'DeLonghi'
#             keyword_dict['Topic'] = topic
#             keyword_dict['Topic Frequency'] = topic_keywords_dict[topic]['frequency']
#             keyword_dict['Keyword'] = keyword[0]
#             keyword_dict['Keyword Weight'] = keyword[1]
#             topic_df = topic_df.append(keyword_dict, ignore_index=True)
